...
"""
Unleashed - v00024
- New: Auto-tab-naming — sets terminal tab title to "REPONAME YYYY-MM-DD HH:MM"
  before Claude PTY spawns, so multiple sessions are visually distinguishable
- Carries forward all v23 features: typeahead dedup, thinking animation filter,
...
class Unleashed:
    ...
    def run(self):
        ...
        _start_meta = {"version": VERSION}
        if self.profile_name:
            _start_meta["profile"] = self.profile_name
        _telemetry_emit("session.start", repo=_repo_name, metadata=_start_meta)
        ...
        try:
            ...
            if self.profile_name:
                _end_meta["profile"] = self.profile_name
            _telemetry_emit("session.end", repo=_repo_name, metadata=_end_meta)
            _telemetry_flush()

            sys.exit(0)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=f"Unleashed v{VERSION} — Telemetry Hooks + Auto-Tab-Naming + Shared Filter + Rate-Limited Mirror + Sentinel")
