...
- Fix: Reduce idle CPU ~213 wakeups/sec -> ~10 (#134)
  ...
  - F3: Resize poll interval 0.3s -> 2.0s
- Carries forward all v30 features: auto-onboard, session index, --sessions,
...
# v31: WaitForSingleObject constants (#134 — event-driven stdin)
WAIT_OBJECT_0 = 0
WAIT_TIMEOUT = 258
...
class Unleashed:
    ...
    def _reader_stdin(self, pty):
        ...
        while self.running:
            try:
                # v31: Block until input available (100ms timeout for shutdown check) (#134)
                wait_result = kernel32.WaitForSingleObject(self.stdin_handle, 100)
                if wait_result != WAIT_OBJECT_0:
                    continue  # timeout or error — loop back to check self.running
                ...
                for i in range(events_read.value):
                    ...
                    if vk == 0x43 and (left_ctrl or right_ctrl):
                        self.running = False
                        self._done_event.set()  # v31: signal main loop (#134)
                        return
    ...
    def _resize_monitor(self, pty):
        ...
        while self.running and pty.isalive():
            try:
                ...
                time.sleep(2.0)  # v31: resize is rare, 2s latency is fine (#134)
            except Exception as e:
    ...
    def _reader_pty(self, pty):
        while self.running and pty.isalive():
            try:
                ...
                self._done_event.set()  # v31: signal main loop (#134)
                break
        self._done_event.set()  # v31: PTY exited normally (#134)
    ...
    def run(self):
        ...
        try:
            # v31: event-driven wait replaces spin-wait (#134)
