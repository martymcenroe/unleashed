...
"""
Unleashed - v00028
- New: Auto-handoff — compaction emergency trigger + configurable timer reminder (#91)
  - Detects 'auto-compact' in PTY stream → injects /handoff (last chance before context loss)
  - Timer-based reminder after N minutes (default 120): "remind" (stderr) or "inject" (/handoff)
  - Configurable via .unleashed.json handoff key
- Carries forward all v27 features: auto-config, auto-pickup, companion cleanup,
  per-repo session logs, tab focus-back, console tab, telemetry hooks, auto-tab-naming,
  typeahead dedup, thinking animation filter, partial render filter, CUF space fix,
  short-line filter, spinner fragment filter, sentinel gate, shared garbage filter,
  rate-limited mirror
...
class Unleashed:
    ...
    def _handoff_timer_callback(self):
        ...
        if self.handoff_timer_action == "inject":
            sys.stderr.write(f"\n\033[33m[unleashed] Session at {elapsed_m}m — auto-injecting /handoff\033[0m\n")
            sys.stderr.flush()
            self._inject_handoff("timer")
        else:
            # "remind" mode — just warn
            sys.stderr.write(f"\n\033[33m[unleashed] Session at {elapsed_m}m — consider /handoff\033[0m\n")
            sys.stderr.flush()
            log(f"Handoff reminder fired at {elapsed_m}m (remind mode)")
