...
"""
Unleashed - v00029
- Fix: Timer reminder uses tab title instead of stderr — no TUI collision (#102)
  - "remind" mode: updates Windows Terminal tab title with handoff indicator
  - "inject" mode: unchanged (writes to PTY input)
  - Tab title reverts are not needed — session ends shortly after handoff
- Carries forward all v28 features: auto-handoff (compaction trigger + timer reminder),
  auto-config, auto-pickup, companion cleanup, per-repo session logs, tab focus-back,
...
class Unleashed:
    ...
    def _load_repo_config(self):
        """Read .unleashed.json from target repo and apply CLI-level config (#57).
        ...
        try:
            ...
            if model:
                ...
                sys.stderr.write(f"[v{VERSION}] Config: model={model_id} (profile={profile})\n")
                sys.stderr.flush()
            ...
            if effort:
                ...
                sys.stderr.write(f"[v{VERSION}] Config: effort={effort}\n")
                sys.stderr.flush()
    ...
    def _reader_pty(self, pty):
        while self.running and pty.isalive():
            try:
                ...
                # v27: Auto-pickup — inject /pickup after first prompt (#68)
                if self.pickup and not self._pickup_sent:
                    if b'\xe2\x9d\xaf' in clean_chunk:  # ❯ (UTF-8)
                        self._pickup_sent = True
                        log("Detected initial prompt (❯), sending /pickup")
                        time.sleep(0.5)
                        pty.write('/pickup\r')
    ...
    def run(self):
        ...
        # v27: Read .unleashed.json before PTY spawn (#57)
