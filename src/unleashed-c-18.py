#!/usr/bin/env python3
"""
Unleashed - v00018 "Triplet Tabs"
- New: Session Mirror tab (--mirror) — timestamped, colorized scrollable transcript
- New: Permission Friction Logger tab (--friction) — structured permission prompt data
- New: Joint log mode (--joint-log) — permissions inline in session mirror
- Carries forward all v17 fixes: ANSI stripping, resize forwarding, multi-pattern matching

GitHub Issue: https://github.com/martymcenroe/unleashed/issues/5
"""
import os
import sys
import threading
...
class Unleashed:
    ...
    def _auto_answer(self, pty):
        ...
        self.in_approval = True
        self.overlap_buffer = b""
        time.sleep(0.8)  # slightly longer than permission — let the UI render
        pty.write('1\r')
        log("Sent '1\\r' to PTY")
        time.sleep(0.5)
        self.in_approval = False
    ...
    def run(self):
        ...
        try:
            ...
            # Close loggers before PTY termination
            if self.session_logger:
