...
class Unleashed:
    ...
    def _log_to_mirror(self, raw_data):
        ...
        v21: Uses shared is_garbage() from transcript_filters module — same 95
        patterns used by post-session clean_transcript.py. Replaces the old
        4-pattern NOISE_RE + heuristic approach.

        Pipeline:
          1. mirror_strip_ansi() — cursor-tracking parser, spaces at word gaps
          2. Split into lines
          3. Strip orphaned CSI/OSC payloads (split across PTY reads)
          4. is_garbage(stripped) — shared 95-pattern filter
          5. Dedup against recent 32 lines
        """
        ...
        try:
            ...
            for line in text.split('\n'):
                ...
                # Normalize: strip spinner prefix for dedup comparison
