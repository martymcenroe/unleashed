...
v18 changes:
- Fix "1" echo in reply window: send just CR (Enter) instead of '1\r'
- Add post-write sleep so in_approval stays set during UI transition
- Read terminal size before console mode change (race fix from Claude version)
"""
import os
import sys
import threading
...
class UnleashedG:
    ...
    def do_approval(self, pty):
        ...
        v17 sent '1\\r' which echoed "1" into Gemini's reply window.
        Just CR works — the confirmation dialog has a default selection.
        Post-write sleep keeps in_approval set while the UI transitions.
        """
    ...
    def run(self):
        ...
        # incorrect dimensions after mode change.
        try:
