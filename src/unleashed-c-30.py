...
"""
Unleashed - v00030
- New: Auto-onboard — injects /onboard after first prompt instead of /pickup (#129)
  - Default on: `onboard.auto` in .unleashed.json (--no-onboard to disable)
  - --pickup now injects /onboard --pickup (skip detection, go straight to import)
  - Startup echo: model/effort/window/assemblyZero to stderr + log
- New: Session index — captures last 10 lines at session end to data/session-index.jsonl
  - --sessions flag: display session index and exit (no Claude launch)
  - Enables thrashing detection and crash recovery in /onboard
- New: Reads assemblyZero and onboard.* config from .unleashed.json
- Carries forward all v29 features: auto-handoff (compaction trigger + tab-title timer),
  auto-config, companion cleanup, per-repo session logs, tab focus-back,
  console tab, telemetry hooks, auto-tab-naming, typeahead dedup, thinking animation filter,
  partial render filter, CUF space fix, short-line filter, spinner fragment filter,
  sentinel gate, shared garbage filter, rate-limited mirror
...
# Issue #50: Telemetry hooks — import from AssemblyZero
try:
    from assemblyzero.telemetry import emit as _telemetry_emit, flush as _telemetry_flush
except ImportError:
    # AssemblyZero not on path — try relative to script location
    _az_root = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "AssemblyZero")
    if os.path.isdir(_az_root):
        sys.path.insert(0, _az_root)
        try:
            from assemblyzero.telemetry import emit as _telemetry_emit, flush as _telemetry_flush
        except ImportError:
            _telemetry_emit = lambda *a, **kw: None
            _telemetry_flush = lambda: 0
    else:
        _telemetry_emit = lambda *a, **kw: None
        _telemetry_flush = lambda: 0
...
class SessionLogger:
    def __init__(self, session_ts: str, joint_log: bool, log_dir: str = None, project: str = None):
        if log_dir and project:
            self.path = os.path.join(log_dir, f"session-{project}-{session_ts}.log")
        else:
...
class FrictionLogger:
    def __init__(self, session_ts: str, session_id: str, log_dir: str = None, project: str = None):
        if log_dir and project:
            self.human_path = os.path.join(log_dir, f"friction-{project}-{session_ts}.log")
            self.jsonl_path = os.path.join(log_dir, f"friction-{project}-{session_ts}.jsonl")
        else:
    ...
    def record_prompt(self, pattern_matched: str, raw_context: str, event_type: str = "permission_prompt"):
        ...
        # v25: Telemetry — approval event
        _telemetry_emit("approval.permission", metadata={
...
class Unleashed:
    def __init__(self, cwd=None, mirror=False, friction=False, joint_log=False,
                 ...
                 home_tab=0, pickup=False, auto_onboard=True, sessions=False,
                 claude_args=None):
        ...
        self.joint_log = joint_log
        # v26: console tab and focus-back
        self.console = console
        self.home_tab = home_tab
        # v27: auto-pickup / v30: auto-onboard
        self.pickup = pickup
        self.auto_onboard = auto_onboard
        self.sessions = sessions
        self._init_cmd_sent = False
        # v30: config fields read from .unleashed.json
        self.assembly_zero = False
        self.onboard_config = {}
        # v28: auto-handoff (compaction trigger + timer reminder)
        ...
        log(f"Initialized v{VERSION} in {self.cwd} (mirror={mirror}, friction={friction}, joint_log={joint_log}, sentinel_shadow={sentinel_shadow}, sentinel_scope={sentinel_scope}, pickup={pickup}, auto_onboard={auto_onboard}, claude_args={self.claude_args})")
    ...
    def _load_repo_config(self):
        ...
        Adds --model and --effort flags to claude_args before PTY spawn.
        Reads assemblyZero and onboard config for auto-onboard injection.
        """
        ...
        try:
            ...
            if model:
                ...
                self.claude_args = ['--model', model_id] + self.claude_args
                log(f"Repo config: model={model_id} (profile={profile})")
            ...
            if 'reminder_minutes' in handoff_config:
                self.handoff_reminder_minutes = int(handoff_config['reminder_minutes'])
            if 'timer_action' in handoff_config:
                action = handoff_config['timer_action']
                if action in ('remind', 'inject'):
                    self.handoff_timer_action = action
            if 'compaction_trigger' in handoff_config:
                self.handoff_compaction_trigger = bool(handoff_config['compaction_trigger'])
            if handoff_config:
                log(f"Repo config: handoff reminder={self.handoff_reminder_minutes}m, "
                    f"action={self.handoff_timer_action}, "
                    f"compaction={self.handoff_compaction_trigger}")

            # v30: assemblyZero and onboard config (#129)
            self.assembly_zero = config.get('assemblyZero', False)
            onboard_config = config.get('onboard', {})
            self.onboard_config = onboard_config
            # Override auto_onboard from config (CLI --no-onboard takes precedence)
            ...
            if onboard_config:
                log(f"Repo config: onboard auto={self.auto_onboard}, "
                    ...
                    f"assemblyZero={self.assembly_zero}")
        except Exception as e:
    ...
    def _reader_pty(self, pty):
        while self.running and pty.isalive():
            try:
                ...
                # v30: Auto-onboard — inject /onboard after first prompt (#129)
                # Replaces v27 auto-pickup: --pickup now sends /onboard --pickup
    ...
    def _inject_handoff(self, source: str):
        """Inject /handoff into PTY stream. Single-shot — only fires once per session.

        Args:
            source: "compaction" or "timer" — for logging
        """
        if self._handoff_sent or not self._handoff_pty:
            return
        self._handoff_sent = True
        log(f"Auto-handoff triggered ({source}), injecting /handoff")
        try:
            time.sleep(1.0)  # longer delay than pickup — let current tool finish
            self._handoff_pty.write('/handoff\r')
            log(f"Auto-sent /handoff ({source})")
        except Exception as e:
            log(f"WARNING: Failed to inject /handoff ({source}): {e}")

    def _handoff_timer_callback(self):
        """Called by threading.Timer after reminder_minutes. Either warns or injects.

        v29 fix (#102): "remind" mode updates the Windows Terminal tab title instead
        of writing to stderr, which collided with the Claude TUI input area and was
        immediately overwritten by TUI redraws.
        """
        elapsed_m = int((time.time() - self.session_start) / 60) if self.session_start else 0
        if self.handoff_timer_action == "inject":
            # inject mode: update tab title then inject /handoff
            repo_name = os.path.basename(self.cwd).upper() if self.cwd else "SESSION"
            sys.stdout.write(f'\033]0;\u26a0 HANDOFF {elapsed_m}m \u2014 {repo_name}\007')
            sys.stdout.flush()
            log(f"Handoff inject: tab title updated, injecting /handoff at {elapsed_m}m")
            self._inject_handoff("timer")
        else:
            # "remind" mode — update tab title as persistent visual indicator
            repo_name = os.path.basename(self.cwd).upper() if self.cwd else "SESSION"
            sys.stdout.write(f'\033]0;\u26a0 HANDOFF {elapsed_m}m \u2014 {repo_name}\007')
            sys.stdout.flush()
            log(f"Handoff reminder fired at {elapsed_m}m (remind mode) — tab title updated")
    ...
    def _display_sessions(self):
        """Display session index and exit (#129). Called when --sessions flag is set."""
        index_path = Path(self.cwd) / 'data' / 'session-index.jsonl'
        if not index_path.exists():
            sys.stderr.write("No session index found. Run at least one session first.\n")
            sys.exit(1)
        entries = []
    ...
    def _capture_session_index(self):
        """Capture last 10 lines from session mirror to session-index.jsonl (#129)."""
        if not self.session_logger or not self.session_logger.path:
            return
        mirror_path = self.session_logger.path
        try:
            if not os.path.exists(mirror_path):
                return
            # Read last 10 non-empty, non-header lines from mirror
            with open(mirror_path, 'r', encoding='utf-8', errors='replace') as f:
                all_lines = f.readlines()
            # Filter: skip empty lines, header/footer separators
            meaningful = [l.rstrip() for l in all_lines
                          if l.strip() and not l.strip().startswith('=' * 10)]
            last_10 = meaningful[-10:] if meaningful else []
            ...
            record = {
                ...
                "line_count": self.session_logger.line_count,
                "project": Path(self.cwd).name
            }
            ...
            log(f"Session index: captured {len(last_10)} lines, {self.session_logger.line_count} total, {duration_min}m")
        except Exception as e:
    ...
    def run(self):
        ...
        # v30: load config early for --sessions display (#129)
        self._load_repo_config()

        # v30: --sessions flag — display session index and exit
        if self.sessions:
        ...
        # v27/v30: .unleashed.json already loaded at top of run() (#57, #129)

        # v24: Set tab title before PTY spawns
        self._set_tab_title()
        ...
        # v25: Telemetry — session.start after PTY spawns
        _repo_name = os.path.basename(self.cwd) if self.cwd else "unknown"
        _telemetry_emit("session.start", repo=_repo_name, metadata={"version": VERSION})
        ...
        self._handoff_pty = pty  # v28: for auto-handoff injection
        session_id = f"unleashed-{session_ts}"

        # v26: Compute per-repo log directory (same as raw transcripts)
        _log_dir = str(Path(self.cwd) / 'data' / 'unleashed')
        _project = Path(self.cwd).name
        os.makedirs(_log_dir, exist_ok=True)

        if self.sentinel_shadow:
            shadow_path = os.path.join(_log_dir, f"sentinel-shadow-{_project}-{session_ts}.log")
        ...
        if self.mirror:
            self.session_logger = SessionLogger(session_ts, self.joint_log,
                                                log_dir=_log_dir, project=_project)
        ...
        if self.mirror or self.friction or self.console:
            time.sleep(0.3)  # let tabs render before switching focus
        ...
        # v28: Start handoff reminder timer
        if self.handoff_reminder_minutes > 0:
            delay_s = self.handoff_reminder_minutes * 60
            self._handoff_timer = threading.Timer(delay_s, self._handoff_timer_callback)
            self._handoff_timer.daemon = True
            self._handoff_timer.start()
            log(f"Handoff timer started: {self.handoff_reminder_minutes}m "
                f"(action={self.handoff_timer_action})")
        ...
        try:
            ...
            self.running = False
            # v28: cancel handoff timer if still pending
            if self._handoff_timer:
            ...
            # v30: capture session index before closing loggers (#129)
            ...
            # v25: Telemetry — session.end with metrics
            _friction_count = self.friction_logger.prompt_count if self.friction_logger else 0
            _telemetry_emit("session.end", repo=_repo_name, metadata={
                "version": VERSION,
                "duration_s": int(elapsed),
                "approval_count": self.approval_count,
                "friction_prompts": _friction_count,
            })
            _telemetry_flush()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=f"Unleashed v{VERSION} — Auto-Handoff + Auto-Config + Auto-Pickup + Per-Repo Logs + Telemetry + Sentinel")
    ...
    parser.add_argument("--pickup", action="store_true", help="Auto-send /onboard --pickup after Claude initializes (skip detection)")
    parser.add_argument("--no-onboard", action="store_false", dest="auto_onboard", help="Disable auto-onboard injection at session start")
    parser.add_argument("--sessions", action="store_true", help="Display session index and exit (no Claude launch)")
