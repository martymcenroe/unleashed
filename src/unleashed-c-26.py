...
"""
Unleashed - v00026
- New: Per-repo session logs — mirror, friction, and sentinel shadow save to
  {target_repo}/data/unleashed/ with project name in filename (#65)
- New: Tab focus-back — companion tabs no longer steal focus from Claude session (#66)
- New: Console tab — optional bash tab cd'd to target repo (#67)
- Carries forward all v25 features: telemetry hooks, auto-tab-naming, typeahead dedup,
  thinking animation filter, partial render filter, CUF space fix, short-line filter,
  spinner fragment filter, sentinel gate, shared garbage filter, rate-limited mirror
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=f"Unleashed v{VERSION} — Per-Repo Logs + Tab Focus + Console Tab + Telemetry + Sentinel")
