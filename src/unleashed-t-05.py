#!/usr/bin/env python3
"""
Unleashed-T - v05 (alpha)
Codex CLI wrapper with PTY handling, live session logs, and companion tabs.

v05 changes (#149):
- Native wrapper flags for Codex launch-time resume/fork workflows
- Launch-mode-aware logging for fresh, resume, and fork sessions

v04 changes: t-03 + event-driven loop, session index, resize 2s
...
# v04: WaitForSingleObject constants (event-driven stdin)
WAIT_OBJECT_0 = 0
WAIT_TIMEOUT = 258
...
class SessionLogger:
    def __init__(self, session_ts: str, log_dir: str = "logs", project: str = "codex",
        ...
        self.fh.write(f"  Mode: {launch_mode}\n")
        self.fh.write(f"{'=' * 60}\n\n")
        self.fh.flush()
        self.line_count = 6
...
class FrictionLogger:
    def __init__(self, session_ts: str, log_dir: str = "logs", project: str = "codex",
        ...
        self.fh_human.write(f"  Mode: {launch_mode} (approval detection deferred to v02)\n")
        self.fh_human.write(f"{'=' * 60}\n\n")
...
class UnleashedT:
    def __init__(self, cwd=None, mirror=True, friction=True, console=True, home_tab=0,
                 codex_args=None, sessions=False, resume=False, resume_last=False,
                 resume_id=None, fork=False, fork_last=False, fork_id=None):
        ...
        self._done_event = threading.Event()  # v04: event-driven main loop
        self.sessions = sessions  # --sessions flag
        self.resume = resume
        self.resume_last = resume_last
        self.resume_id = resume_id
        self.fork = fork
        self.fork_last = fork_last
        self.fork_id = fork_id
        self.launch_mode = "codex -a never -s workspace-write"
        self._mirror_buffer = b""
        self._mirror_last_flush = 0.0
        self._mirror_recent = deque(maxlen=32)
        self._pending_prompt = None

    def _build_codex_invocation(self):
        base_cmd = [
            "cmd", "/c", CODEX_CMD,
        ...
        launch_args = []
        launch_label = "codex -a never -s workspace-write"
        if self.resume or self.resume_last or self.resume_id:
            launch_args = ["resume"]
            launch_label = "codex resume -a never -s workspace-write"
            if self.resume_last:
                launch_args.append("--last")
                launch_label = "codex resume --last -a never -s workspace-write"
            elif self.resume_id:
                launch_args.append(self.resume_id)
                launch_label = f"codex resume {self.resume_id} -a never -s workspace-write"
        elif self.fork or self.fork_last or self.fork_id:
            launch_args = ["fork"]
            launch_label = "codex fork -a never -s workspace-write"
            if self.fork_last:
                launch_args.append("--last")
                launch_label = "codex fork --last -a never -s workspace-write"
            elif self.fork_id:
                launch_args.append(self.fork_id)
                launch_label = f"codex fork {self.fork_id} -a never -s workspace-write"
        self.launch_mode = launch_label
        if launch_args:
            return base_cmd + launch_args + self.codex_args
        return base_cmd + self.codex_args
    ...
    def _capture_session_index(self):
        ...
        try:
            ...
            meaningful = [line.rstrip() for line in all_lines if line.strip() and not line.strip().startswith('=' * 10)]
    ...
    def run(self):
        ...
        codex_cmd = self._build_codex_invocation()
        log(f"Launch mode: {self.launch_mode}")

        self.session_logger = SessionLogger(
            session_ts, log_dir=_log_dir, project=_project, launch_mode=self.launch_mode
        )
        ...
        env["GIT_CONFIG_COUNT"] = "1"  # Sandbox strips schannel credentials (#109, #111)
        ...
        try:
            ...
            sys.stderr.write(f"  Launch:       {self.launch_mode}\n")
            ...
            sys.stderr.write("----------------------------------------------------------\n")
            sys.stderr.flush()
            log(f"Session summary: {dur}, launch={self.launch_mode}, approval=never sandbox=workspace-write")
...
if __name__ == "__main__":
    ...
    mode_group.add_argument('--sessions', action='store_true', help='Show wrapper-local recent sessions and exit')
    mode_group.add_argument('--resume', action='store_true', help='Launch Codex native resume picker')
    mode_group.add_argument('--resume-last', action='store_true', help='Resume the most recent Codex session')
    mode_group.add_argument('--resume-id', metavar='SESSION', help='Resume a specific Codex session ID or thread name')
    mode_group.add_argument('--fork', action='store_true', help='Launch Codex native fork picker')
    mode_group.add_argument('--fork-last', action='store_true', help='Fork the most recent Codex session')
    mode_group.add_argument('--fork-id', metavar='SESSION', help='Fork a specific Codex session ID')
    args, codex_args = parser.parse_known_args()
    UnleashedT(cwd=args.cwd, mirror=args.mirror, friction=args.friction,
               console=args.console, home_tab=args.home_tab, codex_args=codex_args,
               sessions=args.sessions, resume=args.resume, resume_last=args.resume_last,
               resume_id=args.resume_id, fork=args.fork, fork_last=args.fork_last,
               fork_id=args.fork_id).run()
