#!/usr/bin/env python3
"""
Unleashed Danger Test - PTY Wrapper for Claude Code Auto-Approval (Test Version)

Test version of unleashed-danger with --debug flag for verbose logging.
...
- Single Escape key passes through (user can interrupt Claude)

Usage:
  python tools/unleashed-danger-test.py [--debug] [--dry-run] [--help]
...
VERSION = "1.4.1-danger-t"  # Danger test version - deploy changes here first
...
if "--version" in sys.argv or "-v" in sys.argv:
    print(f"unleashed-danger-test.py {VERSION}")
    sys.exit(0)

try:
    import winpty
except ImportError:
    print("[UNLEASHED-DANGER-TEST] Error: pywinpty not installed. Run: poetry add pywinpty", file=sys.stderr)
    sys.exit(1)
...
class InputReader:
    """Non-blocking input reader - DANGER MODE with selective filtering.

    This reader filters out terminal response escape sequences (multi-char)
    while allowing single Escape keypresses through for user cancellation.
    """
    ...
    def _is_safe_char(self, char: str) -> bool:
        ...
        (Only filtering chars users never type standalone)
        """
        if not char:
            return False
        code = ord(char)
            ...
            try:
                if HAS_MSVCRT:
                    if msvcrt.kbhit():
                        ...
                        # FILTER: Drop terminal response garbage (brackets, semicolons)
                        # These only appear standalone from terminal responses, not user input
                        # NOTE: digits, 'c', '?' removed - users need to type them!
                        if char in ';[]':
                            # Likely terminal response garbage - drop it
                            debug_log(f"input: FILTERED (terminal response char)")
                            # Drain any remaining garbage
                            while msvcrt.kbhit():
                                garbage = msvcrt.getwch()
                                debug_log(f"input: draining garbage: {repr(garbage)}")
                                if garbage in '\r\n':  # Stop at newline
                                    break
                            continue
                        ...
                        if char in ('\xe0', '\x00'):
                            ...
                            if scan in key_map:
                                ...
                                debug_log(f"input: mapped to {repr(key_map[scan])}")
                            else:
                                debug_log(f"input: unknown scan code")
                        elif char == '\x1b':
                            # Escape character - might be terminal response or user keypress
                            # Wait briefly to see if more characters follow (escape sequence)
                            time.sleep(0.02)  # 20ms - enough for terminal sequences
                            seq = char
                            while msvcrt.kbhit():
                                seq += msvcrt.getwch()
                            debug_log(f"input: escape_seq={repr(seq)}")

                            # Filter multi-char escape sequences (terminal garbage)
                            # but allow single Escape key through for user cancellation
                            if len(seq) > 1:
                                # Multi-char escape sequence - definitely terminal response
                                debug_log(f"input: FILTERED (multi-char escape)")
                                continue
                            # Single escape - pass it through so user can cancel Claude
                            debug_log(f"input: queued single escape")
                            self.queue.put('\x1b')
                        elif self._is_safe_char(char):
                            self.queue.put(char)
                            debug_log(f"input: queued safe char")
                        else:
                            debug_log(f"input: FILTERED (unsafe)")
...
class EventLogger:
    ...
    def __init__(self, log_dir: Path, session_id: str):
        ...
        # Raw output log
        self.raw_log_path = log_dir / f"unleashed-danger-test_{session_id}.log"
        self.raw_log = open(self.raw_log_path, 'wb')

        # Structured event log
        self.event_log_path = log_dir / f"unleashed-danger-test_events_{session_id}.jsonl"
...
class CountdownOverlay:
    ...
    def show(self, seconds_remaining: int):
        ...
        self.active = True
        message = f"{CURSOR_SAVE}{CURSOR_HOME}{BOLD}{RED}[UNLEASHED-DANGER-TEST]{YELLOW} Auto-approving in {seconds_remaining}s... (Press any key to cancel){RESET}{CLEAR_LINE}{CURSOR_RESTORE}"
        self.writer(message)
    ...
    def show_approved(self):
        ...
        message = f"{CURSOR_SAVE}{CURSOR_HOME}{BOLD}{RED}[UNLEASHED-DANGER-TEST]{YELLOW} Auto-approved!{RESET}{CLEAR_LINE}{CURSOR_RESTORE}"
        self.writer(message)

    def show_cancelled(self):
        """Show cancellation message briefly."""
        message = f"{CURSOR_SAVE}{CURSOR_HOME}{BOLD}{RED}[UNLEASHED-DANGER-TEST]{YELLOW} Cancelled by user{RESET}{CLEAR_LINE}{CURSOR_RESTORE}"
        self.writer(message)
...
class Unleashed:
    """Main PTY wrapper for auto-approval - DANGER MODE TEST.

    DANGER MODE means:
    - NO safety checks (hard block, dangerous path, git destructive)
    - Single Escape passes through (user can interrupt Claude)
    - Filters multi-char escape sequences (terminal garbage)
    ...
    def _handle_countdown(self) -> bool:
        ...
        self.logger.log_event("COUNTDOWN_START", delay=self.delay, mode="DANGER-TEST")
        debug_log(f"starting countdown: {self.delay}s (DANGER MODE)")
    ...
    def _show_banner(self):
        ...
        banner = f"{BOLD}{RED}[UNLEASHED-DANGER-TEST v{VERSION}]{debug_mode}{YELLOW} Auto-approval | {self.delay}s | NO SAFETY CHECKS | Dir: {self.cwd}{RESET}\n"
    ...
    def run(self):
        ...
        self.logger.log_event("START", version=VERSION, delay=self.delay, dry_run=self.dry_run, mode="DANGER-TEST", debug=DEBUG)
    ...
    def _cleanup(self):
        ...
        if self.logger:
            ...
            self.logger.close()
            print(f"\n[UNLEASHED-DANGER-TEST] Logs saved to: {self.logger.log_dir}", file=sys.stderr)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Unleashed Danger Test - Auto-approval wrapper for Claude Code (NO SAFETY CHECKS - Test Version)",
...
Examples:
  python tools/unleashed-danger-test.py              # Danger test mode
  python tools/unleashed-danger-test.py --debug      # With debug output
  python tools/unleashed-danger-test.py --dry-run    # Test detection without injection
  UNLEASHED_DELAY=5 python tools/unleashed-danger-test.py  # 5-second countdown
...
WARNING:
  ...
  - Single Escape passes through (can interrupt Claude)
  ...
  Use at your own risk.
    ...
    try:
        ...
        print(f"[UNLEASHED-DANGER-TEST] Fatal error: {e}", file=sys.stderr)
        sys.exit(1)
